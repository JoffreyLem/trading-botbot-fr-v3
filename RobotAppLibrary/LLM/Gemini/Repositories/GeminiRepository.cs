    ...
    IOptions<GeminiSettings> geminiSettings) : ILLMRepository
{
    ...
    public async Task<AnalyseMarcheResponse?> Analyse(AnalyseMarcheRequest analyseMarcheRequest)
    {
        string jsonMarcheRequest = JsonSerializer.Serialize(analyseMarcheRequest);
        ...
        string prompt = $@"
                         ...
                         1. Analyse les données de trading graphique et les indicateurs techniques présents dans les 'Données à analyser'.
                         2. Sur la base de ton analyse, détermine un sentiment de marché sur une échelle de 0 à 100, où :
                            - 0 indique un fort sentiment de vente.
                           - 50 indique un sentiment neutre.
                            - 100 indique un fort sentiment d'achat.
                         3. Détermine des niveaux de Stop Loss et de Take Profit pertinents en fonction des données analysées.
                         4. Formate ta réponse **uniquement** comme un objet JSON sérialisé de type AnalyseMarcheResponse.
                         5. Ne retourne **rien d'autre** que cet objet JSON. Pas de texte avant, pas de texte après, pas d'explication supplémentaire.
